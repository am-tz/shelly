...
from infra.server.requests.text_input_request import TextInputRequest, ChatQueues
from infra.server.requests.text_output_request import TextOutputRequest
...
class App:
    ...
    def __init__(self, move_implementation: MoveImp, exit_flag: Event, chat_queues: ChatQueues):
        ...
        add_resource(self.__api, TextInputRequest, queues=chat_queues)
        add_resource(self.__api, TextOutputRequest, queues=chat_queues)
        add_resource(self.__api, ExitRequest, exit_flag=exit_flag)
        add_resource(self.__api, ShutdownRequest)


if __name__ == '__main__':
    App(MoveImp(), Event(), ChatQueues()).app.run(debug=True)
